...
def question1():
    if request.method == 'POST':
        ...
        if 'radio_yes' not in request.form and 'radio_no' not in request.form:
            ...
            answer.append(f"{satisfaction_survey.questions[0].question} - {radio_yes}")
        else:
            answer.append(f"{satisfaction_survey.questions[0].question} - {radio_no}")
...
def question2():
    if request.method == 'POST':
        ...
        if radio_yes is not None:
            answer.append(f"{satisfaction_survey.questions[1].question} - {radio_yes}")
        else:
            answer.append(f"{satisfaction_survey.questions[1].question} - {radio_no}")
...
def question3():
    if request.method == 'POST':
        ...
        if radio_less is not None:
            answer.append(f"{satisfaction_survey.questions[2].question} - {radio_less}")
        else:
            answer.append(f"{satisfaction_survey.questions[2].question} - {radio_more}")
